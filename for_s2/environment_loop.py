...
class EnvironmentLoop(core.Worker):
    ...
    def __init__(
        ...
        self._logger = loggers.make_default_logger(label)
        self._should_update = should_update
        self._observers = observers

    def run_episode(self,episode_count) -> loggers.LoggingData:
        """Run one episode.
        ...
        # Generate an action from the agent's policy and step the environment.
            ...
            if self._should_update and episode_count <= 4000:
                self._actor.update()
    ...
    def run(self,
        ...
        test = False
        self._environment._config.task_minimum_data_size = self._environment._config.task_minimum_data_size*0.3
        self._environment._config.task_maximum_data_size = self._environment._config.task_maximum_data_size*0.3
        self._environment._config.task_minimum_delay_thresholds =  self._environment._config.task_minimum_delay_thresholds*0.3
        self._environment._config.task_minimum_delay_thresholds =  self._environment._config.task_maximum_delay_thresholds*0.3
        new_computing_speeds = [10.0 * 1e9, 10.0 * 1e9, 10.0 * 1e9, 10.0 * 1e9, 6.0 * 1e9, 3.0 * 1e9, 3.0 * 1e9, 3.0 * 1e9, 3.0 * 1e9]
        for i in range(9):
            self._environment._edge_list._edge_list[i]._computing_speed = new_computing_speeds[i]
        with signals.runtime_terminator():
            while not should_terminate(episode_count, step_count):
                start_time = time.time()
                self._environment._occuiped = True
                self._environment._config.task_seed  = episode_count
                
                result = self.run_episode(episode_count)
                results.append(result)
                episode_count += 1
                step_count += result['episode_length']
                # Log the given episode results.
                self._logger.write(result)
                print(f"episode:{episode_count} reward:{result['episode_return'][-1]} time:{time.time()-start_time} ")
                if episode_count == 4100 :     
                
                    np.save(f"result/mad4pg_randomtask_occur_actor10_{np.random.rand()}",results)
