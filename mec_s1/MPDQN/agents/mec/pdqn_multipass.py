...
from agents.mec.pdqn import PDQNAgent
...
class MultiPassQActor(nn.Module):
    ...
    def __init__(self, state_size, action_size, action_parameter_size_list, hidden_layers=(100,),
        ...
        self.action_parameter_size = 648
    ...
    def forward(self, state, action_parameters):
        ...
        # with torch.no_grad():
        x = torch.cat((state, torch.zeros_like(action_parameters).cuda()), dim=1)
        x = x.repeat(self.action_size, 1)
        for a in range(self.action_size):
            x[a*batch_size:(a+1)*batch_size, self.state_size + int(self.offsets[a]): self.state_size + int(self.offsets[a+1])] \
                = action_parameters[:, int(self.offsets[a]):int(self.offsets[a+1])]
...
class MultiPassPDQNAgent(PDQNAgent):
    ...
    def __init__(self,
        ...
        self.actor = MultiPassQActor(26, 27, 648,
                                     **kwargs['actor_kwargs']).to(device)
        self.actor_target = MultiPassQActor(26, 27, 648,
                                            **kwargs['actor_kwargs']).to(device)
