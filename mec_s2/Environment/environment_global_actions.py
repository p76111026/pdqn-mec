...
class vehicularNetworkEnv(dm_env.Environment):
    ...
    def step(self, action: np.ndarray):
        ...
        self._reward, cumulative_reward, average_vehicle_SINR, average_vehicle_intar_interference, average_vehicle_inter_interference, \
            average_vehicle_interference, average_transmision_time, average_wired_transmission_time, average_execution_time, average_service_time, successful_serviced_number, task_offloaded_number, task_required_number = self.compute_reward(action)
    ...
    def compute_reward(
        ...
        for edge_index in range(self._config.edge_number):
            ...
            for i in range(int(tasks_number_within_edge)):
                ...
                #if task_assignment_value == 0:
                #    processing_edge_index=edge_index
                #if task_assignment_value == 1:
                #    processing_edge_index=(edge_index+1)%9
                #if task_assignment_value == 2:
                #    processing_edge_index=(edge_index-1)%9        
                #if task_assignment_value == 3:
                #    processing_edge_index=(edge_index+2)%9
                #if task_assignment_value == 4:
                #    processing_edge_index=(edge_index-2)%9      
                #if task_assignment_value > 2 : 
                #    processing_edge_index = int(task_assignment_value)-10    
                
                #processing_edge_index= int(task_assignment_value)
            ...
            for i in range(task_sum):
                ...
                if self._occuiped:
                    if edge_computing_speed - edge_occupied_computing_speed <= 0:
                        ...
                        vehicle_edge_computation_resources[vehicle_index][edge_index] = computation_resource_allocation[i] * (edge_computing_speed - edge_occupied_computing_speed)
                else:
                    vehicle_edge_computation_resources[vehicle_index][edge_index] = computation_resource_allocation[i] * edge_computing_speed
                task_index = self._vehicle_list.get_vehicle_by_index(vehicle_index).get_requested_task_by_slot_index(self._time_slots.now())
                data_size = self._task_list.get_task_by_index(task_index).get_data_size()
