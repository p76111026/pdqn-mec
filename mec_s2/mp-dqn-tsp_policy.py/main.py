...
def run(seed, episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold, replay_memory_size,
    ...
    np.random.seed(0)
    # new_computing_speeds = np.random.uniform(env._config.edge_minimum_computing_cycles/3, env._config.edge_maximum_computing_cycles*0.8, env._config.edge_number)        
    # for i in range(9):
    ...
    for episode_num in range(episodes):
        ...
        while not timestep.last():
            ...
            next_state = next_state.reshape(-1, 44)
            #for i in range(9):
    ...
    result['state'].append(state_history)    
    np.save("mp_dqn_tsp_s2",result)
