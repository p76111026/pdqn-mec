...
class ParamActor(nn.Module):
    ...
    def forward(self, state):
        ...
        action_params = action_params.tanh()
        action_params = (1+action_params)/2
...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        self.action_parameter_sizes = np.array([24 for i in range(1,self.num_actions+1)])
        self.action_parameter_size = int(self.action_parameter_sizes.sum())
        self.action_max = torch.from_numpy(np.ones((self.num_actions,))).float().to(device)
        self.action_min = -self.action_max.detach()
        self.action_range = (self.action_max-self.action_min).detach()
        #print([self.action_space.spaces[i].high for i in range(1,self.num_actions+1)])
        self.action_parameter_max_numpy = np.concatenate([[1.0] for i in range(1,self.num_actions+1)]).ravel()
        self.action_parameter_min_numpy = np.concatenate([[0.0] for i in range(1,self.num_actions+1)]).ravel()
    ...
    def act(self, state):
        with torch.no_grad():
            ...
            if rnd < self.epsilon:
                action = self.np_random.choice(self.num_actions,9)
                if not self.use_ornstein_noise:
                    all_action_parameters = torch.from_numpy(np.random.uniform(np.zeros(3000),np.ones(3000),size=(9,3000)))
            else:
            ...
            offset = [np.array([self.action_parameter_sizes[i] for i in range(j)], dtype=int).sum() for j in action]
            if self.use_ornstein_noise and self.noise is not None:
                for i in range(len(action)):
                    all_action_parameters[i,offset[i]:offset[i] + self.action_parameter_sizes[action[i]]] += self.noise.sample()[offset:offset + self.action_parameter_sizes[action]]
            action_parameters = [all_action_parameters[i,offset[i]:offset[i]+self.action_parameter_sizes[action[i]]] for i in range(len(action))]
    ...
    def _optimize_td_loss(self):
        ...
        self.q_loss_history.append(Q_loss.data.cpu().numpy())
        self.loss_q_history.append(loss_Q.data.cpu().numpy())
    ...
    def save_models(self, prefix):
        ...
        :return:
        """
        torch.save(self.actor.state_dict(), prefix + '_actor123.pt')
        torch.save(self.actor_param.state_dict(), prefix + '_actor_param.pt')
        torch.save(self.loss_q_history,prefix+'_loss_q')
        torch.save(self.q_loss_history,prefix+'_q_loss')
