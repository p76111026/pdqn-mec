import sys 
sys.path.append("/home/yee/Game-Theoretic-Deep-Reinforcement-Learning/MPDQN")
import torch
...
class QActor(nn.Module):
    ...
    def forward(self, state, action_parameters):
        ...
        x = torch.cat((state, action_parameters.cuda()), dim=1)
...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        self.action_parameter_sizes = np.ones(27)*24 
        self.action_parameter_size = int(self.action_parameter_sizes.sum())
        self.action_max = torch.from_numpy(np.ones((self.num_actions,))).float().to(device)
        self.action_min = -self.action_max.detach()
        self.action_range = (self.action_max-self.action_min).detach()
        
        self.action_parameter_max_numpy = np.concatenate([np.ones(24) for i in range(1,self.num_actions+1)]).ravel()
        self.action_parameter_min_numpy = np.concatenate([np.zeros(24) for i in range(1,self.num_actions+1)]).ravel()
        ...
        with torch.no_grad():
            ...
            if rnd < self.epsilon:
                ...
                if not self.use_ornstein_noise:
                    ...
                    all_action_parameters = all_action_parameters.to(torch.float32)
            else:
            ...
            action_parameters = all_action_parameters[offset:offset+int(self.action_parameter_sizes[action])]
